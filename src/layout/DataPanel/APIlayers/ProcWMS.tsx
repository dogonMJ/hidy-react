...
import { getMarks, noTileCached, is3D } from "Utils/UtilsApi";
...
const propsWMTS = (api: Api, time: string, key: string, elevation: number) => {
  return {
    type: api.type,
    url: api.url,
    params: {
...
const ProcWMS = (props: Urls) => {
  ...
  if (noTileCached(tileProps, key)) {
    if (api.type === 'wmts') {
      tileProps.push(propsWMTS(api, time, key, depth))
    } else {
